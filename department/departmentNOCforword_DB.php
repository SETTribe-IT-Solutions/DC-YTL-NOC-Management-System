...
if (isset($_POST['forwardNOC'])) {
    ...
    $query = "UPDATE departmentNocApplications
              SET inspectionOfficer = '$inspectionOfficer', HODremark = '$HODremark'
              WHERE applicationId = '$applicationId'";

    if (mysqli_query($conn, $query)) {
        // ✅ Redirect with success status
        header("Location: ../department/department_Report.php?status=success");
        exit();
    } else {
        // ✅ Redirect with error status and error message
        $error = urlencode(mysqli_error($conn));
        header("Location: ../department/department_Report.php?status=error&msg=$error");
        exit();
    }
}
?>
